...
(define (queue-empty? q)
  (if (null? (append (queue-left q) (queue-right q)))
      #t
      #f))
...
; Veți întoarce coada actualizată.
(define (dequeue q)
  (struct-copy queue q [left (if (null? (queue-left q))
                              (cdr(reverse(queue-right q))) 
                              (cdr (queue-left q)))]
                       [right (if (= (queue-size-l q) 0)
                              '()
                              (queue-right q))]
                       [size-l (if (= (queue-size-l q) 0)
                              (- (queue-size-r q) 1)
                              (- (queue-size-l q) 1))]
                       [size-r (if (= (queue-size-l q) 0)
                              0
                             (queue-size-r q))]))
