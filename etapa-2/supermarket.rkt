  ...
  (λ (name n-items)
    (λ (C)
      (if (null? (counter-queue C))
               (struct-copy counter C [index (counter-index C)] [tt (+ (counter-tt C) n-items)] [et (+ (counter-et C) n-items)] [queue (append (counter-queue C) (list(cons name n-items)))])
               (struct-copy counter C [index (counter-index C)] [tt (+ (counter-tt C) n-items)] [et (+ (counter-et C) 0)] [queue (append (counter-queue C) (list(cons name n-items)))])
          ))))
...
(define (remove-first-from-counter C)
    (struct-copy counter C [tt (- (counter-tt C) (counter-et C))]
                 [et (if (null? (cdr(counter-queue C))) 0 (cdr(car(cdr(counter-queue C)))))]
      ...
      (match (car requests)
        ...
        [(list 'ensure average)
           (if (>= average (/ (foldr (λ(x sum-tt) (+ sum-tt (counter-tt x))) 0 (append fast-counters slow-counters))
              ...
              (serve (cdr requests) fast-counters (add-counter average fast-counters slow-counters))
         )]
        ...
        [(list 'remove-first)
            (if (null? (filter (λ (x) (not(null? (counter-queue x)))) (append fast-counters slow-counters)))
                (serve (cdr requests) fast-counters slow-counters)
                (if (null? (filter (λ (x) (not(null? (counter-queue x)))) fast-counters))
                    (serve (cdr requests)
                     in-et      fast-counters
                           (foldr (λ (x acc) (if (= (car(min-et(filter (λ(y) (< 0 (counter-et y))) slow-counters))) (counter-index x))
                                                 (cons (remove-first-from-counter x) acc)
                                                 (cons  x acc))) '() slow-counters))
                    (if (null? (filter (λ (x) (not(null? (counter-queue x)))) slow-counters))
                        (serve (cdr requests)
                           (foldr (λ (x acc) (if (= (car(min-et(filter (λ(y) (< 0 (counter-et y))) fast-counters))) (counter-index x))
                                                 (cons (remove-first-from-counter x) acc)
                                                 (cons  x acc))) '() fast-counters)
                           slow-counters)
                        (if (<= (counter-index (car slow-counters)) (car(min-et(filter (λ(x) (not(null? (counter-queue x)))) (append fast-counters slow-counters)))))
                                (serve (cdr requests)
                                 fast-counters
                                 (foldr (λ (x acc) (if (= (car(min-et(filter (λ(y) (not(null? (counter-queue y)))) slow-counters))) (counter-index x))
                                                 (cons (remove-first-from-counter x) acc)
                                                 (cons  x acc))) '() slow-counters))
                                (serve (cdr requests)
                                 (foldr (λ (x acc) (if (= (car(min-et(filter (λ(y) (not(null? (counter-queue y)))) fast-counters))) (counter-index x))
                                                 (cons (remove-first-from-counter x) acc)
                                                 (cons  x acc))) '() fast-counters)
                                  slow-counters)
                        ))))])))
