...
;(define (verify counters index mintt)       
  ;    (if  (null? counters)
  ;         (cons index mintt)
  ;         (if (< (counter-tt (car counters)) mintt)
  ;             (verify (cdr counters) (counter-index (car counters)) (counter-tt (car counters)))
  ;             (verify (cdr counters) index mintt))))

;(define (min-tt counters)
 ;  (verify counters (counter-index (car counters)) (counter-tt (car counters))))

(define min-tt
  (λ (counters)
     (foldl (λ (x acc) (if (< (counter-tt x) (cdr acc))
                           (cons (counter-index x) (counter-tt x))
                           acc))
            (cons (counter-index (car counters)) (counter-tt ( car counters))) counters)))
...
(define (add-to-counter C name n-items)
     (define Cnou (struct-copy counter C [index (counter-index C)] [tt (+ (counter-tt C) n-items)] [queue (append (counter-queue C) (list(cons name n-items)))]))
Cnou)
