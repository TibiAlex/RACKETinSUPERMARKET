...
(define (rotate left right Acc)
      (cond 
          ((and (null? right) (stream-empty? left)) Acc)
          ((and (null? right) (not (stream-empty? left))) 
              (stream-cons (stream-first left) (rotate (stream-rest left) '()  Acc)))
          ((and (not(null? right)) (stream-empty? left))
              (rotate empty-stream (cdr right) (stream-cons (car right) Acc)))
          (else
...
(define (enqueue x q)
   (if (> (queue-size-l q) (queue-size-r q))
        ...
        (struct-copy queue q [left (rotate (queue-left q) (cons x (queue-right q)) empty-stream)]
                     [right '()]
                     [size-l (+ (+  1 (queue-size-r q)) (queue-size-l q))]
                     [size-r 0]))
   )
